...
impl<E: PairingEngine> SNARK<E> for VOProofR1CS {
  ...
  fn prove(pk: &Self::PK, x: &Self::Ins, w: &Self::Wit) -> Result<Self::Pf, Error> {
    ...
    init_size!(cap_s_c, cdensity, size);
    init_size!(ell, input_size, size);
    define!(vovectorsize, cap_k + cap_s_a + cap_s_b + cap_s_c);
    ...
    redefine_zero_pad_concat_vector!(u_vec_1, vovectorsize, delta_vec);
    define_commit_vector!(cm_u_vec_1, u_vec_1, pk.powers, vovectorsize + 1);
    ...
    define_commit_vector!(cm_r_vec_tilde, r_vec_tilde, pk.powers, vovectorsize + 1);
    ...
    define_vec!(
      ...
      vector_concat!(
        ...
        expression_vector!(
          i,
          c_1
            * range_index!(1, cap_k, i + vovectorsize)
            * range_index!(1, cap_k, minus_i64!(i + vovectorsize, 1))
            + c_10
              * vector_index!(h_vec, minus_i64!(i + vovectorsize, 1))
              * vector_index!(pk.v_vec, minus_i64!(i + vovectorsize, cap_k + 1))
            + c_10
              * vector_index!(h_vec, minus_i64!(i + vovectorsize, 3 * cap_h + 1))
              * vector_index!(s_vec, i + vovectorsize)
            + c_11
              * range_index!(1, vovectorsize, minus_i64!(i + vovectorsize, 1))
              * (vector_index!(r_vec_tilde, minus_i64!(i + vovectorsize, 1))
                - vector_index!(r_vec_tilde, minus_i64!(i + vovectorsize, 2)))
            + c_2
              * vector_index!(h_vec, minus_i64!(i + vovectorsize, 1))
              * (c_3 * vector_index!(pk.w_vec, minus_i64!(i + vovectorsize, cap_k + 1))
                + c_4 * range_index!(1, ell_1, minus_i64!(i + vovectorsize, cap_k + 1))
                + c_5 * vector_index!(pk.u_vec, minus_i64!(i + vovectorsize, cap_k + 1))
                + vector_index!(pk.y_vec, minus_i64!(i + vovectorsize, cap_k + 1)))
            + c_6
              * power(
                range_index!(1, ell_1, minus_i64!(i + vovectorsize, cap_k + 1)),
                2
              )
            + c_7
              * vector_index!(
                u_vec_1,
                minus_i64!(i + vovectorsize, -cap_h + vovectorsize + 1)
              )
              * vector_index!(
                u_vec_1,
                minus_i64!(i + vovectorsize, -2 * cap_h + vovectorsize + 1)
              )
            + c_8
              * range_index!(
                1,
              ...
              * vector_index!(
                u_vec_1,
                minus_i64!(i + vovectorsize, -3 * cap_h + vovectorsize + 1)
              )
            + c_9
              * vector_index!(u_vec_1, minus_i64!(i + vovectorsize, 1))
              * vector_index!(s_vec, i + vovectorsize)
            - range_index!(1, 3 * cap_h, minus_i64!(i + vovectorsize, 1))
              * range_index!(1, 3 * cap_h, i + vovectorsize)
            + vector_index!(s_vec, i + vovectorsize)
              * (mu * range_index!(1, 3 * cap_h, minus_i64!(i + vovectorsize, 1))
                - power_vector_index!(gamma, 3 * cap_h, minus_i64!(i + vovectorsize, 1)))
            + vector_index!(h_vec, i + vovectorsize)
              * (c * range_index!(1, cap_k, minus_i64!(i + vovectorsize, 1))
                + c_1 * power_vector_index!(gamma, cap_k, minus_i64!(i + vovectorsize, 1))),
          maxshift + 2
        )
      )
    );
    ...
    define_expression_vector!(
      ...
      c_10
        * vector_index!(
          v_vec_8,
          minus_i64!(i - maxshift - vovectorsize, cap_k - shiftlength_7 + 1)
        )
        + c_14
          * vector_index!(
            v_vec_1,
            minus_i64!(i - maxshift - vovectorsize, cap_k - shiftlength + 1)
          )
        + c_15
          * vector_index!(
            v_vec_3,
            minus_i64!(i - maxshift - vovectorsize, cap_k - shiftlength_2 + 1)
          )
        + c_16
          * vector_index!(
            v_vec_5,
            minus_i64!(i - maxshift - vovectorsize, -cap_h - shiftlength_4 + 1)
          )
        + c_17
          * vector_index!(
            v_vec_7,
            minus_i64!(i - maxshift - vovectorsize, -3 * cap_h - shiftlength_6 + 1)
          )
        + c_18
          * vector_index!(
            r_vec_tilde,
            minus_i64!(i - maxshift - vovectorsize, 2 - vovectorsize)
          )
        + c_19
          * power_vector_index!(
            c_12,
            ell + 1,
            minus_i64!(i - maxshift - vovectorsize, 1 - ell)
          )
        + c_19 * vector_index!(v_vec_18, minus_i64!(i - maxshift - vovectorsize, 2 - ell))
        + c_2
          * vector_index!(
            v_vec_4,
            minus_i64!(i - maxshift - vovectorsize, cap_k - shiftlength_3 + 1)
          )
        + c_20
          * vector_index!(
            v_vec_9,
            minus_i64!(i - maxshift - vovectorsize, 2 - 3 * cap_h)
          )
        + c_21
          * vector_index!(
            v_vec_10,
        ...
        + c_22
          * vector_index!(
            v_vec_11,
            minus_i64!(
              i - maxshift - vovectorsize,
              -cap_k - cap_s_a - cap_s_b - cap_s_c + 2
            )
          )
        + c_23 * vector_index!(v_vec_12, minus_i64!(i - maxshift - vovectorsize, 2 - cap_k))
        + c_24 * vector_index!(v_vec_13, minus_i64!(i - maxshift - vovectorsize, 2 - cap_k))
        + c_25
          * vector_index!(
            v_vec_14,
            minus_i64!(i - maxshift - vovectorsize, -cap_k - ell_1 + 2)
          )
        + c_26
          * vector_index!(
            v_vec_15,
            minus_i64!(i - maxshift - vovectorsize, cap_k - shiftlength_1 + 1)
          )
        + c_27
          * vector_index!(
            v_vec_16,
        ...
        + c_28
          * vector_index!(
            v_vec_17,
            minus_i64!(i - maxshift - vovectorsize, -3 * cap_h - ell + 1)
          )
        + c_29
          * vector_index!(
            v_vec_19,
        ...
        + c_30
          * vector_index!(
            ...
            minus_i64!(
              i - maxshift - vovectorsize,
              -cap_k - cap_s_a - cap_s_b - cap_s_c + 3
            )
          )
        + c_31
          * vector_index!(
        ...
        + c_33 * vector_index!(v_vec_22, minus_i64!(i - maxshift - vovectorsize, 2 - cap_k))
        + c_34
          * vector_index!(
            v_vec_23,
            minus_i64!(
        ...
        + c_9
          * vector_index!(
            v_vec_6,
            minus_i64!(i - maxshift - vovectorsize, 1 - shiftlength_5)
          ),
      maxshift + vovectorsize
    );
    define_expression_vector!(
      h_vec_3,
      i,
      c_10 * vector_index!(v_vec_8, minus_i64!(i + 1, cap_k - shiftlength_7 + 1))
        ...
        + c_18 * vector_index!(r_vec_tilde, minus_i64!(i + 1, 2 - vovectorsize))
    ...
    define_commit_vector!(cm_h_vec_3, h_vec_3, pk.powers, maxshift + vovectorsize);
    get_randomness_from_hash!(
      z,
      one!(),
      x_vec,
    ...
    define_eval_vector_expression!(y, omega / z, i, vector_index!(h_vec, i), vovectorsize + 1);
    ...
    define!(
      ...
      power(alpha, 5)
        * (alpha
          ...
          * (omega * power(omega / z, ell) - one!() * z)
          + y_1
            * power(z, -2 * cap_h + vovectorsize)
            * power(omega / z, -cap_h + cap_k + cap_s_a + cap_s_b + cap_s_c)
            * (omega - one!() * z)
          + power(z, -3 * cap_h + vovectorsize + 1)
            * power(omega / z, -cap_h + cap_k + cap_s_a + cap_s_b + cap_s_c)
    ...
    define_vec_mut!(
      ...
      expression_vector!(
        ...
        linear_combination_base_zero!(
          ...
          c_45,
          vector_index!(h_vec_2, -cap_d + i + maxshift + vovectorsize),
          c_46,
