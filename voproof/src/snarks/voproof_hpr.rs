...
impl<E: PairingEngine> SNARK<E> for VOProofHPR {
  ...
  fn prove(pk: &Self::PK, x: &Self::Ins, w: &Self::Wit) -> Result<Self::Pf, Error> {
    ...
    redefine_zero_pad_concat_vector!(w_vec_1, vovectorsize, delta_vec);
    define_commit_vector!(cm_w_vec_1, w_vec_1, pk.powers, vovectorsize + 1);
    ...
    define_vec!(
      ...
      vector_concat!(
        ...
        expression_vector!(
          i,
          c_1
            * range_index!(1, 3 * cap_k + 1, minus_i64!(i + vovectorsize, 1))
            * range_index!(1, 3 * cap_k + 1, i + vovectorsize)
            + c_10
              * vector_index!(h_vec, minus_i64!(i + vovectorsize, 1))
              * vector_index!(pk.v_vec, minus_i64!(i + vovectorsize, 3 * cap_k + 2))
            + c_10
              * vector_index!(h_vec, minus_i64!(i + vovectorsize, cap_h + 1))
            ...
            + c_2
              * vector_index!(h_vec, minus_i64!(i + vovectorsize, 1))
              * (c_3 * vector_index!(pk.w_vec, minus_i64!(i + vovectorsize, 3 * cap_k + 2))
                + c_4 * range_index!(1, ell, minus_i64!(i + vovectorsize, 3 * cap_k + 2))
                + c_5 * vector_index!(pk.u_vec, minus_i64!(i + vovectorsize, 3 * cap_k + 2))
                + vector_index!(pk.y_vec, minus_i64!(i + vovectorsize, 3 * cap_k + 2)))
            + c_6
              * power(
                range_index!(1, ell, minus_i64!(i + vovectorsize, 3 * cap_k + 2)),
                2
              )
            + c_7
              * vector_index!(
                w_vec_1,
                minus_i64!(i + vovectorsize, -cap_k + vovectorsize + 1)
              )
              * vector_index!(
                w_vec_1,
                minus_i64!(i + vovectorsize, -2 * cap_k + vovectorsize + 1)
              )
            + c_8
              * range_index!(
                1,
              ...
              * vector_index!(
                w_vec_1,
                minus_i64!(i + vovectorsize, -3 * cap_k + vovectorsize + 1)
              )
            + vector_index!(h_vec, i + vovectorsize)
              * (c * range_index!(1, 3 * cap_k + 1, minus_i64!(i + vovectorsize, 1))
                + c_1 * power_vector_index!(gamma, 3 * cap_k + 1, minus_i64!(i + vovectorsize, 1)))
            - range_index!(1, cap_h, minus_i64!(i + vovectorsize, 1))
              * range_index!(1, cap_h, i + vovectorsize)
            + vector_index!(s_vec, i + vovectorsize)
              * (mu * range_index!(1, cap_h, minus_i64!(i + vovectorsize, 1))
                - power_vector_index!(gamma, cap_h, minus_i64!(i + vovectorsize, 1)))
            + vector_index!(s_vec, i + vovectorsize)
              * (power(alpha, 6) * delta!(i + vovectorsize, cap_h + 1)
                + c_9 * vector_index!(x_vec, minus_i64!(i + vovectorsize, 1))
                + c_9 * vector_index!(w_vec_1, minus_i64!(i + vovectorsize, cap_h + 2))),
          maxshift + 2
        )
      )
    );
    ...
    define_expression_vector!(
      ...
      c_10
        * vector_index!(
          v_vec_9,
          minus_i64!(i - maxshift - vovectorsize, 3 * cap_k - shiftlength_8 + 2)
        )
        + c_14 * vector_index!(s_vec, minus_i64!(i - maxshift - vovectorsize, 1 - cap_h))
        + c_15
          * vector_index!(
            v_vec_1,
            minus_i64!(i - maxshift - vovectorsize, 3 * cap_k - shiftlength + 2)
          )
        + c_16
          * vector_index!(
            v_vec_3,
            minus_i64!(i - maxshift - vovectorsize, 3 * cap_k - shiftlength_2 + 2)
          )
        + c_17
          * vector_index!(
            v_vec_5,
            minus_i64!(i - maxshift - vovectorsize, -cap_k - shiftlength_4 + 1)
          )
        + c_18
          * vector_index!(
            v_vec_7,
            minus_i64!(i - maxshift - vovectorsize, -cap_h - shiftlength_6)
          )
        + c_19
          * vector_index!(
            v_vec_8,
            minus_i64!(i - maxshift - vovectorsize, -cap_h - shiftlength_7 + 1)
          )
        + c_2
          * vector_index!(
            v_vec_4,
            minus_i64!(i - maxshift - vovectorsize, 3 * cap_k - shiftlength_3 + 2)
          )
        + c_20
          * vector_index!(
            r_vec_tilde,
        ...
        + c_21 * vector_index!(v_vec_10, minus_i64!(i - maxshift - vovectorsize, 2 - cap_h))
        + c_22 * vector_index!(v_vec_11, minus_i64!(i - maxshift - vovectorsize, 2 - cap_h))
        + c_23
          * vector_index!(
            v_vec_12,
            minus_i64!(
        ...
        + c_24
          * vector_index!(
            v_vec_13,
            minus_i64!(i - maxshift - vovectorsize, 1 - 3 * cap_k)
          )
        + c_25
          * vector_index!(
            v_vec_14,
        ...
        + c_26
          * vector_index!(
            v_vec_15,
            minus_i64!(i - maxshift - vovectorsize, -3 * cap_k - ell + 1)
          )
        + c_27
          * vector_index!(
            v_vec_16,
            minus_i64!(i - maxshift - vovectorsize, 3 * cap_k - shiftlength_1 + 2)
          )
        + c_28
          * vector_index!(
            v_vec_17,
            minus_i64!(i - maxshift - vovectorsize, 2 - 3 * cap_k)
          )
        + c_29
          * vector_index!(
            v_vec_18,
        ...
        + c_32 * vector_index!(v_vec_20, minus_i64!(i - maxshift - vovectorsize, 2 - cap_h))
        + c_33
          * vector_index!(
            v_vec_21,
            minus_i64!(i - maxshift - vovectorsize, 1 - 3 * cap_k)
        ...
        + c_34
          * vector_index!(
            ...
            minus_i64!(
              i - maxshift - vovectorsize,
              -cap_s_a - cap_s_b - cap_s_c - cap_s_d + 2
            )
          )
        + c_9
          * vector_index!(
    ...
    define_expression_vector!(
      ...
      c_10 * vector_index!(v_vec_9, minus_i64!(i + 1, 3 * cap_k - shiftlength_8 + 2))
        ...
        + c_20 * vector_index!(r_vec_tilde, minus_i64!(i + 1, 2 - vovectorsize))
    ...
    define!(
      ...
      power(alpha, 5)
        ...
        * (y_1 * power(z, -2 * cap_k + vovectorsize) * (omega - one!() * z)
          + power(z, -3 * cap_k + vovectorsize + 1) * (one!() - power(omega / z, cap_k)))
        / (omega - one!() * z)
    );
    define!(
      c_42,
