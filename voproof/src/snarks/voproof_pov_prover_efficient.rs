...
impl<E: PairingEngine> SNARK<E> for VOProofPOVProverEfficient {
  ...
  fn prove(pk: &Self::PK, x: &Self::Ins, w: &Self::Wit) -> Result<Self::Pf, Error> {
    ...
    define!(n, 3 * cap_c);
    define_concat_subvec!(u_vec, a_vec, 0, cap_c, b_vec, 0, cap_c_m);
    define_concat_subvec!(v_vec, b_vec, cap_c_m, cap_c, c_vec, 0, cap_c_a + cap_c_m);
    ...
    define_accumulate_vector_mul!(
      ...
      mul!(
        vector_index!(u_vec, minus_i64!(i, 1))
          + vector_index!(v_vec, minus_i64!(i, cap_c + cap_c_m + 1))
          + vector_index!(pk.d_vec, minus_i64!(i, 2 * cap_c + cap_c_a + cap_c_m + 1))
        ...
        expression_vector!(
          ...
          c * vector_index!(t_vec, minus_i64!(i + n, 1))
            * (vector_index!(u_vec, minus_i64!(i + n, 1))
              + vector_index!(v_vec, minus_i64!(i + n, cap_c + cap_c_m + 1))
              - vector_index!(x_vec, minus_i64!(i + n, 1)))
            + c_2
              * vector_index!(r_vec, minus_i64!(i + n, -3 * cap_c + n + 1))
              * (vector_index!(u_vec, minus_i64!(i + n, -3 * cap_c + n + 1))
                + vector_index!(v_vec, minus_i64!(i + n, -2 * cap_c + cap_c_m + n + 1))
                + vector_index!(
                  pk.d_vec,
                  minus_i64!(i + n, -cap_c + cap_c_a + cap_c_m + n + 1)
                )
            ...
            - vector_index!(v_vec, minus_i64!(i + n, cap_c_m + 1))
              * range_index!(1, cap_c_m, minus_i64!(i + n, cap_c + 1))
            + vector_index!(u_vec, minus_i64!(i + n, cap_c + 1)) * vector_index!(u_vec, i + n)
            + (power(alpha, 3) * delta!(i + n, -3 * cap_c + n + 1)
    ...
    define_vector_reverse_omega_shift!(v_vec_9, r_vec, omega, shiftlength_7);
    ...
    define_vector_power_mul!(v_vec_11, v_vec, c_3, cap_c_m);
    define_vector_power_mul!(v_vec_12, v_vec, c_3, cap_c_a);
    define_vector_power_mul!(v_vec_13, u_vec, c_3, cap_c_a);
    define_vector_power_mul!(v_vec_14, v_vec_9, gamma, 3 * cap_c);
    define_vector_power_mul!(v_vec_15, v_vec_9, one!(), 3 * cap_c);
    define_vector_power_mul!(v_vec_16, t_vec_1, c_3, cap_c + cap_c_m + 1);
    define!(c_4, power(omega, cap_c));
    ...
    define!(c_11, -power(omega, cap_c + cap_c_m - 1));
    define!(c_12, alpha * power(omega, cap_c + cap_c_a - 1));
    define!(c_13, -alpha * power(omega, cap_c + cap_c_a - 1));
    define!(c_14, power(alpha, 3) * omega * zeta);
    define!(c_15, power(alpha, 3) * omega * zeta_1);
    define!(c_16, -power(alpha, 3) * zeta_1);
    define!(c_17, -power(omega, 4 * cap_c + cap_c_m));
    define_expression_vector!(
      h_vec_1,
      i,
      -power(alpha, 4) * power(omega, 3 * cap_c - 1) * delta!(i - maxshift - n, 1)
        + c * vector_index!(v_vec_3, minus_i64!(i - maxshift - n, 1 - shiftlength_1))
        + c
          * vector_index!(
            v_vec_4,
            minus_i64!(i - maxshift - n, cap_c + cap_c_m - shiftlength_2 + 1)
          )
        + c_1 * vector_index!(v_vec, minus_i64!(i - maxshift - n, cap_c + cap_c_m + 1))
        + c_1 * vector_index!(u_vec, minus_i64!(i - maxshift - n, 1))
        + c_1
          * vector_index!(
            pk.d_vec,
            minus_i64!(i - maxshift - n, 2 * cap_c + cap_c_a + cap_c_m + 1)
          )
        + c_10 * range_index!(1, 3 * cap_c, minus_i64!(i - maxshift - n, 1))
        + c_11 * vector_index!(v_vec_11, minus_i64!(i - maxshift - n, 2 - cap_c))
        + c_12 * vector_index!(v_vec_12, minus_i64!(i - maxshift - n, 2 - cap_c_a))
        + c_12
          * vector_index!(
            v_vec_13,
            minus_i64!(i - maxshift - n, -cap_c_a - cap_c_m + 2)
          )
        + c_13 * vector_index!(v_vec_12, minus_i64!(i - maxshift - n, -cap_c - cap_c_a + 2))
        + c_14 * vector_index!(v_vec_14, minus_i64!(i - maxshift - n, -shiftlength_7))
        + c_15 * vector_index!(v_vec_15, minus_i64!(i - maxshift - n, -shiftlength_7))
        + c_16 * vector_index!(v_vec_15, minus_i64!(i - maxshift - n, 1 - shiftlength_7))
        + c_17 * vector_index!(v_vec_16, minus_i64!(i - maxshift - n, -cap_c - cap_c_m))
        + c_2 * vector_index!(v_vec_6, minus_i64!(i - maxshift - n, 1 - shiftlength_4))
        + c_2
          * vector_index!(
            v_vec_7,
            minus_i64!(i - maxshift - n, cap_c + cap_c_m - shiftlength_5 + 1)
          )
        + c_2
          * vector_index!(
            v_vec_8,
            minus_i64!(
              i - maxshift - n,
              2 * cap_c + cap_c_a + cap_c_m - shiftlength_6 + 1
            )
          )
        + c_4
          * vector_index!(
        ...
        + c_5 * vector_index!(v_vec_5, minus_i64!(i - maxshift - n, 1 - shiftlength_3))
        + c_6 * vector_index!(v_vec_6, minus_i64!(i - maxshift - n, -shiftlength_4))
        + c_6
          * vector_index!(
            v_vec_7,
            minus_i64!(i - maxshift - n, cap_c + cap_c_m - shiftlength_5)
          )
        + c_6
          * vector_index!(
            v_vec_8,
            minus_i64!(
              i - maxshift - n,
              2 * cap_c + cap_c_a + cap_c_m - shiftlength_6
            )
          )
        + c_7
          * vector_index!(
            v_vec_9,
            minus_i64!(i - maxshift - n, 3 * cap_c - shiftlength_7)
          )
        + c_8 * vector_index!(v_vec_10, minus_i64!(i - maxshift - n, 1 - shiftlength_8))
    ...
    define_expression_vector!(
      h_vec_2,
      i,
      -power(alpha, 4) * power(omega, 3 * cap_c - 1) * delta!(i + 1, 1)
        + c * vector_index!(v_vec_3, minus_i64!(i + 1, 1 - shiftlength_1))
        + c
          * vector_index!(
            v_vec_4,
            minus_i64!(i + 1, cap_c + cap_c_m - shiftlength_2 + 1)
          )
        + c_1 * vector_index!(v_vec, minus_i64!(i + 1, cap_c + cap_c_m + 1))
        + c_1 * vector_index!(u_vec, minus_i64!(i + 1, 1))
        + c_1
          * vector_index!(
            pk.d_vec,
            minus_i64!(i + 1, 2 * cap_c + cap_c_a + cap_c_m + 1)
          )
        + c_10 * range_index!(1, 3 * cap_c, minus_i64!(i + 1, 1))
        + c_11 * vector_index!(v_vec_11, minus_i64!(i + 1, 2 - cap_c))
        + c_12 * vector_index!(v_vec_12, minus_i64!(i + 1, 2 - cap_c_a))
        + c_12 * vector_index!(v_vec_13, minus_i64!(i + 1, -cap_c_a - cap_c_m + 2))
        + c_13 * vector_index!(v_vec_12, minus_i64!(i + 1, -cap_c - cap_c_a + 2))
        + c_14 * vector_index!(v_vec_14, minus_i64!(i + 1, -shiftlength_7))
        + c_15 * vector_index!(v_vec_15, minus_i64!(i + 1, -shiftlength_7))
        + c_16 * vector_index!(v_vec_15, minus_i64!(i + 1, 1 - shiftlength_7))
        + c_17 * vector_index!(v_vec_16, minus_i64!(i + 1, -cap_c - cap_c_m))
        + c_2 * vector_index!(v_vec_6, minus_i64!(i + 1, 1 - shiftlength_4))
        + c_2
          * vector_index!(
            v_vec_7,
            minus_i64!(i + 1, cap_c + cap_c_m - shiftlength_5 + 1)
          )
        + c_2
          * vector_index!(
            v_vec_8,
            minus_i64!(i + 1, 2 * cap_c + cap_c_a + cap_c_m - shiftlength_6 + 1)
          )
        + c_4 * vector_index!(v_vec_2, minus_i64!(i + 1, -cap_c - shiftlength + 1))
        + c_5 * vector_index!(v_vec_5, minus_i64!(i + 1, 1 - shiftlength_3))
        + c_6 * vector_index!(v_vec_6, minus_i64!(i + 1, -shiftlength_4))
        + c_6 * vector_index!(v_vec_7, minus_i64!(i + 1, cap_c + cap_c_m - shiftlength_5))
        + c_6
          * vector_index!(
            v_vec_8,
            minus_i64!(i + 1, 2 * cap_c + cap_c_a + cap_c_m - shiftlength_6)
          )
        + c_7 * vector_index!(v_vec_9, minus_i64!(i + 1, 3 * cap_c - shiftlength_7))
        + c_8 * vector_index!(v_vec_10, minus_i64!(i + 1, 1 - shiftlength_8))
    ...
    define!(
      ...
      (power(alpha, 2)
        * (omega - one!() * z)
        * (alpha
          * (-one!() * y_2 * power(z, -2 * cap_c + cap_c_m + n)
            + power(z, -2 * cap_c + cap_c_m + n - 1) * (omega * y_2 + one!() * z))
          + one!() * y_1 * power(z, cap_c + cap_c_m))
        + alpha
          * power(omega / z, cap_c)
          * (one!() - power(omega / z, cap_c_a))
          * (one!() * z - power(z, cap_c + 1))
        + power(z, cap_c_m + 1) * power(omega / z, cap_c) * (one!() - power(omega / z, cap_c_m)))
        / (omega - one!() * z)
    );
    define!(
      c_20,
    ...
    define!(
      c_21,
      power(alpha, 3)
        * (-one!() * y_2 * power(z, -cap_c + cap_c_a + cap_c_m + n)
          + power(z, -cap_c + cap_c_a + cap_c_m + n - 1) * (omega * y_2 + one!() * z))
    );
    define!(c_22, -power(alpha, 3) * power(one!(), 2) * y_2 * zeta);
    define!(
      c_23,
      power(z, n) * power(omega / z, 3 * cap_c) * (one!() - power(omega / z, cap_c + cap_c_m + 1))
        / (omega - one!() * z)
    );
